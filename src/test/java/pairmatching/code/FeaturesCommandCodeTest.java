...
class FeaturesCommandCodeTest {
    ...
    void 존재하지_않는_명령어를_입력하는경우_에러를_발생시킵니다(final String input) {
        assertThatIllegalArgumentException().isThrownBy(() -> InputCommand.of(input))
                .withMessageContaining(FeaturesCommandCode.DID_NOT_EXISTS_CODE);
    }

    @ParameterizedTest
    @ValueSource(strings = {" 1 ", " 2 ", " 3 ", " Q "})
    void 등록된_명령어에_공백이_있더라도_에러가발생하지_않습니다(final String input) {
        var command = InputCommand.of(input);
